...
class ReportScriptTest extends TestCase
{
	...
	private function runReport(string $outdatedFile): array
	{
		$process = proc_open(
			...
			$pipes,
			self::PROJECT,
			['PACKAGIST_URL' => __DIR__ . '/fixtures/p2', 'TARGET_PHP_VERSION' => '8.3', 'COMPATIBILITY_PACKAGES' => 'acme/framework'],
		);
	...
	/**
	 * Check the report is built from the installed list
	 */
	public function testReportsFromTheInstalledList(): void
	{
		...
		$this->assertStringContainsString('`acme/framework ^6`', $stdout);
	}

	/**
	 * Check the report is built from the locked list
	 */
	...
	/**
	 * Check config.platform.php is used over the target version
	 */
	...
	/**
	 * Check unreadable outdated output exits without a report
	 */
